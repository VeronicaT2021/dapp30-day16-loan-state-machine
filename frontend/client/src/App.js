...
function App() {
  ...
  async function updateState() {
    var state = parseInt(await contract.methods.state().call());
    setLoan({ ...loan, state })
  }

  async function fund() {
    await contract.methods.fund()
  ...
  async function reimburse() {
    ...
    await contract.methods.reimburse()
  ...
  return (
    <div className="container">
      ...
      {loan.state > 0 ? (
        <p>End: {(new Date(parseInt(loan.end) * 1000)).getTime()}</p>
      ) : null}
      ...
      {loan.state === 1 && accounts[0].toLowerCase() === loan.borrower.toLowerCase() && isFinished() ? (
        <div className="row">
          <div className="col-sm-12">
            <h2>Reimburse {loan.amount} Wei (principal) + {loan.interest} Wei (interest)</h2>
            <button
              onClick={e => reimburse()}
              type="submit"
              className="btn btn-primary"
